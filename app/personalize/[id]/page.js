...
export default function PersonalizePage({ params }) {
  ...
  useEffect(() => {
    const checkFormCompletionStatus = async () => {
      console.log('🔍 DEBUG: checkFormCompletionStatus called', { 
        user: !!user, 
        personalizationId, 
        formConfig: !!formConfig,
        usedInLessons: formConfig?.usedInLessons 
      });

      if (!user || !personalizationId) {
        console.log('❌ Missing user or personalizationId');
        return;
      }
      ...
      try {
        ...
        if (!formConfig || !formConfig.usedInLessons || formConfig.usedInLessons.length === 0) {
          console.log('❌ No lessons to check, form is not yet active');
          return; // No lessons to check, form is not yet active
        }

        console.log('🔍 Checking form completion for lessons:', formConfig.usedInLessons);

        // Check if TTS requests already exist for any of the lessons this form affects
        const response = await fetch('/api/check-form-completion', {
          method: 'POST',
          headers: {
        ...
        console.log('📥 Form completion check result:', result);
        ...
        if (result && result.alreadyCompleted) {
          ...
          console.log('✅ Form not yet completed, showing form');
        }
      } catch (error) {
    ...
    if (!loading && !loadingConfig) {
      ...
      // Check if user has already completed this personalization form
      // Only check if we have formConfig loaded
      if (formConfig) {
        checkFormCompletionStatus();
      }
    }
  }, [user, loading, loadingConfig, isApproved, router, personalizationId, formConfig, courseFlow]);
